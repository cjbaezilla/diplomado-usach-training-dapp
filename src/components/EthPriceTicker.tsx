...
export function EthPriceTicker() {
  ...
  if (!isHydrated) {
    return (
      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-primary-foreground/5 border border-primary-foreground/10 text-primary-foreground/60 text-xs">
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <span className="font-bold tracking-tight text-white select-none">ETH</span>
      </div>
      <span className={cn(
        "font-bold transition-all duration-300",
        trend === 'up' ? "text-emerald-400" : trend === 'down' ? "text-rose-400" : "text-primary-foreground"
      )}>
